...
- `./batch override <versionId> -m "<msg>"` — reads an override entry from **stdin** (or `--file <path>`, where `--file -` also means stdin); applies it to a **base OR a variant**, creating a new immutable version. On a base the change is baked straight into its content (this is how you tune a base in place — cut its sugar, drop its bake temp). On a variant it's recorded as a component-level delta over the base, inheriting everything else. Entry shapes:
  ...
  - **Atomic multi-op:** pass a **JSON array** of entries instead of one object and they apply as a **single new version** (one commit, one macro recompute). Entries fold **in order**, so a later entry may target a component an earlier one added (e.g. `[{add step s8}, {add usage on s8}, {add note on s8}]`). Prefer this over chaining N `override` calls — no head-id threading, and the history stays one clean commit per logical change.
...
- `./batch list` — all recipes by head version (`name, status, tags, isVariant`, plus `kcalPerServing` / `macroBasis`, plus `tried` / `queued` / `verdict` from the tasting log). `--to-make` filters to the queued (untried) recipes; `--tag <tag>` (exact, case-insensitive) and `--name <substr>` (case-insensitive substring) narrow the list. **Filter by ingredient:** `--with <csv>` keeps only recipes that use ALL the listed ingredients, `--without <csv>` drops any that use ANY of them (refs resolve by id/name/alias; matched on the **flattened** recipe, so a sub-recipe's ingredients count). `--allow-sub` makes both substitution-aware: a recipe that misses a `--with` (or trips a `--without`) is still kept when a **same-`category`** ingredient can stand in, and the in-recipe ingredient to swap is reported per row in `swappable`. So "what can I make with chocolate but no vanilla protein?" is `list --with ing-protein-chocolate --allow-sub` (or `--without ing-protein-vanilla --allow-sub`). Categories are a *plausible-swap* signal, not a macro-equivalence guarantee.
- `./batch tree` — all versions with `derivesFromVersionId` / `prevVersionId` edges (build the forest of bases → variants).
- `./batch ingredient add` — reads a **library ingredient** JSON from stdin (`{ name, macrosPer100g, densityGPerMl?, unitEquivalences?, id? }`); `id` defaults to a slug of the name. Adds/updates the (mutable) library (full-object upsert).
- `./batch ingredient set <ref> [--name --brand --notes --category <fam> --alias a,b --density <gPerMl> --macro k=v --unit k=v]` — **patch an existing** ingredient in place (resolved by id/name/alias). Scalars replace; `--macro` and `--unit` (both repeatable) **merge** onto the existing macros / unitEquivalences, so you can bump one value — `ingredient set ing-egg-whole --unit each=50 --unit serving=50` — without re-sending the whole object. `--category` stamps the substitution family (e.g. `protein-powder`, `sweetener-zero`, `milk`) that powers `list --allow-sub`. The ergonomic for catalog tweaks `ingredient add` lacked.
