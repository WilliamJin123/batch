...
- `./batch ingredient set <ref> [--name --brand --notes --alias a,b --density <gPerMl> --macro k=v --unit k=v]` — **patch an existing** ingredient in place (resolved by id/name/alias). Scalars replace; `--macro` and `--unit` (both repeatable) **merge** onto the existing macros / unitEquivalences, so you can bump one value — `ingredient set ing-egg-whole --unit each=50 --unit serving=50` — without re-sending the whole object. The ergonomic for catalog tweaks `ingredient add` lacked.
