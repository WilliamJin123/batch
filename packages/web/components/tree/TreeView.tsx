...
// far-mode counter-scale: ramps 1→INV_CAP across [1/INV_CAP, FAR_SCALE] so the card's on-screen
// size is CONTINUOUS at the threshold (no size pop), then holds full size and gently shrinks past the cap
const farInv = (s: number) => {
  if (s >= FAR_SCALE) return 1;
  const sFull = 1 / INV_CAP;
  return s <= sFull ? INV_CAP : 1 + (INV_CAP - 1) * (FAR_SCALE - s) / (FAR_SCALE - sFull);
};
  ...
  return (
    <div className="treepage">
      <div className="board" ref={boardRef} onMouseDown={onDown}>
        <div className={`scene${t.scale < FAR_SCALE ? " far" : ""}${smooth ? " smooth" : ""}`} style={{ transform: `translate(${t.ox}px,${t.oy}px) scale(${t.scale})`, width, height, transition: smooth ? "transform .26s cubic-bezier(.22,.61,.36,1)" : "none", ["--inv" as string]: farInv(t.scale) } as React.CSSProperties}>
